...
To start using the drax pipeline, there are three steps described below:
...
This pipeline itself needs no installation - NextFlow will automatically fetch it from GitHub if `will-rowe/drax` is specified as the pipeline name.
...
```bash
wget https://github.com/will-rowe/drax/archive/master.zip
unzip master.zip -d /my-pipelines/
cd /my_data/
nextflow run /my-pipelines/drax-master
```
